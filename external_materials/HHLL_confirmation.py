...
def HHLL_confirmation(df):
    ...
    hBar, hPrice, hCPrice, prevHBar, prevHPrice, prevHCPrice = [0.]*6
    lBar, lPrice, lCPrice, prevLBar, prevLPrice, prevLCPrice = [0.]*6
    ...
    for bar_index in range(1, len(close)):
        ...
        if prevLPrice < lPrice and prevLCPrice < lCPrice and hPrice < high[bar_index] and hCPrice < close[bar_index]:
            if lastTrend != "u":
                firstLBar = prevLBar
                firstLPrice = prevLPrice
            trend = "u"
        elif prevHPrice > hPrice and prevHCPrice > hCPrice and lPrice > low[bar_index] and close[bar_index]:
            if lastTrend != "d":
                firstHBar = prevHBar
                firstHPrice = prevHPrice
            trend = "d"
        elif (trend == "u" and (lCPrice > close[bar_index] or (isHigh_Low(close, open_, bar_index, high=True) and prevHCPrice > hCPrice))) or \
                (trend == "d" and (hCPrice < close[bar_index] or (isHigh_Low(close, open_, bar_index, high=False) and prevLCPrice < lCPrice))):
            trend = ""
        ...
        if trend == "u" and ((lPrice - firstLPrice) / (lBar - firstLBar + 1e-7) < (latestLPrice[-1] - firstLPrice) / (latestLBar[-1] - firstLBar + 1e-7) or prevLBar == firstLBar):
        ...
        if trend == "d" and ((hPrice - firstHPrice) / (hBar - firstHBar + 1e-7) > (latestHPrice[-1] - firstHPrice) / (latestHBar[-1] - firstHBar + 1e-7) or prevHBar == firstHBar):
...
if __name__ == "__main__":
    ...
    out_df = HHLL_confirmation(data)
    
    print("Saving output to HHLL_Strategy.csv")
    out_df.to_csv("./HHLL_Confirmation.csv", index=False)
